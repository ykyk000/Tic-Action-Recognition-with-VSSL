...
def get_accuracy(predicted_actor, actor):
    ...
    predit = predicted_actor.argsort(dim=-1, descending=True)
    # prediction = prediction.view(-1, 1)
    actor = actor.view(-1, 1)
    correct_1 = torch.sum((actor == predit[:,:1]).any(dim=-1).float()).item()
    correct_5 = torch.sum((actor == predit[:,:5]).any(dim=-1).float()).item()
    accuracy_1 = correct_1 / float(predit.shape[0])
    accuracy_5 = correct_5 / float(predit.shape[0])
